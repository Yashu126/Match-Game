...
class App extends Component {
  ...
  getFilteredItems = () => {
    ...
    const filterItems = imagesList.filter(
  ...
  onSelectedImg = id => {
    ...
    if (randomImg.id === id) {
      this.setState(prevState => ({
        ...
        randomImg: imagesList[Math.ceil(Math.random() * imagesList.length)],
      }))
    } else {
      clearInterval(this.timmerId)
      this.setState({isGameon: false})
  ...
  render() {
    const {isGameon, timer, activeTab, score, randomImg} = this.state
    const filterItems = this.getFilteredItems()
    return (
      <div className="background-con">
        <Header score={score} timer={timer} />
        {isGameon ? (
          <div className="img-tab-con">
            <img src={randomImg.imageUrl} alt="match" className="rando-img" />
...
/*
randomImg: imagesList[Math.ceil(Math.random() * tabsList.length)],
<img src={randomImg.imageUrl} alt={randomImg.category} />
 */
